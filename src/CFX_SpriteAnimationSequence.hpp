...
{
  public:
    CFX_SpriteAnimationSequence(uint16_t steps, CFX_Sprite* sprite, int id = 0);
  
    void AddBitmap(const byte* bitmap, uint16_t duration, CFX_SpriteTransition transition);
    void SetBitmap(uint16_t step, const byte* bitmap);
